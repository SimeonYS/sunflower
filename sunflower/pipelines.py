...
class SunflowerPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sunflower`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sunflower WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sunflower (date, title, link, content)"
